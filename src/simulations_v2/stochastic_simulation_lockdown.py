...
class StochasticSimulation:
    ...
    def reset_initial_state(self):
        ...
        self.new_QI_from_contact_tracing = 0
        self.new_QS_from_last_test = 0
        self.new_QS_from_contact_tracing = 0
        self.FP_from_contact_tracing = 0
    ...
    def step_contact_trace(self, new_QI):
        ...
        self.new_QS_from_contact_tracing = total_contacts_quarantined
        self.FP_from_contact_tracing += np.random.binomial(total_contacts_quarantined, self.test_QFPR)

        total_cases_isolated = int(self.cases_isolated_per_contact * resolve_today_QI)

        self.new_QI_from_contact_tracing = total_cases_isolated
    ...
    def step(self):
        ...
        self.missed_cases += new_R_from_mild + new_R_from_severe
    ...
    def _append_sim_df(self):
        ...
        if abs(sum(data) - sum(data[-6:]) - self.pop_size) > 0.0001:
    ...
    def get_current_state_vector(self):
        return np.concatenate([
            ...
            [self.new_real_cases], [self.new_cases], [self.missed_cases]])
    ...
    def get_state_vector_labels(self):
        return ['S', 'QS', 'QI', 'R'] + \
                ...
                ['new_real_cases', 'new_cases', 'missed_cases']
    ...
    def generate_cumulative_stats(self):
        ...
        self.new_real_cases = self.new_QI_from_last_test + self.new_QI_from_contact_tracing + self.new_QI_from_self_reports
        self.new_cases = self.new_QI_from_last_test + self.new_QI_from_contact_tracing + self.new_QI_from_self_reports + self.new_QS_from_last_test + self.FP_from_contact_tracing
