...
def sample_and_save(point_id, point, save_folder, nsamples=100, T=50):
    gc.collect()
    pickle.dump(point, open(save_folder+'point_{}_no_trajectories.dill'.format(point_id), 'wb'))
    sim = map_lhs_point_to_vax_sim(point, param_modifiers)
    inf_trajs_by_group = run_multiple_trajs(sim, T=T, n=nsamples, override_premovein_params=override_params)
    pickle.dump([point, inf_trajs_by_group], open(save_folder+'point_{}_with_trajectories.dill'.format(point_id), 'wb'))
    return [point, inf_trajs_by_group]
...
if __name__ == "__main__":
    ...
    save_folder = 'fall_2021_posterior_projections:{}/'.format(get_timestamp())
    os.mkdir(save_folder)

    posterior_df = load_posterior_df()
    posterior_sample = np.random.multinomial(nsamples, list(posterior_df['posterior']))
    posterior_idxs = []
    for idx, count in enumerate(posterior_sample):
        for _ in range(count):
            posterior_idxs.append(idx)
    
    print("Running sims now, savingn results to {}".format(save_folder))
