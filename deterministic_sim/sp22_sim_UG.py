...
def main(**kwargs):
    ...
    def sim_test_regime(tests_per_week, delay, color):
        ...
        plt.subplot(211)
        # population: plt.plot(np.arange(T)*GENERATION_TIME, s.get_discovered(aggregate=True,cumulative=True), label=label, color=color)
        plt.plot(np.arange(T)*GENERATION_TIME, 
                s.get_total_discovered_for_different_groups(popul.metagroup_indices(['UG'])[0], cumulative=True), 
                label=label, color=color)
        plt.subplot(212)
        # population: plt.plot(np.arange(T)*GENERATION_TIME, s.get_isolated(), label=label, color=color)
        plt.plot(np.arange(T)*GENERATION_TIME, s.get_isolated(popul.metagroup_indices(['UG'])[0]), label=label, color=color)
    ...
    plt.subplot(211)
    # population: plt.plot(np.arange(T)*GENERATION_TIME, s.get_discovered(aggregate=True,cumulative=True), 'k-', label='No surveillance, Discovered')
    plt.plot(np.arange(T)*GENERATION_TIME, 
            s.get_total_discovered_for_different_groups(popul.metagroup_indices(['UG'])[0], cumulative=True), 
            'k-', label='No surveillance, Discovered')
    # population: plt.plot(np.arange(T)*GENERATION_TIME, s.get_infected(aggregate=True,cumulative=True), 'k--', label='No surveillance, Infected')
    plt.plot(np.arange(T)*GENERATION_TIME, 
            s.get_total_infected_for_different_groups(popul.metagroup_indices(['UG'])[0],cumulative=True),
            'k--', label='No surveillance, Infected')
    plt.subplot(212)
    # plt.plot(np.arange(T) * GENERATION_TIME, s.get_isolated(), 'k', label='No surveillance')
    plt.plot(np.arange(T) * GENERATION_TIME, 
            s.get_isolated(popul.metagroup_indices(['UG'])[0]),
            'k', label='No surveillance')
    ...
    plt.savefig('sp22_sim_UG.png', facecolor='w')
    plt.close()
